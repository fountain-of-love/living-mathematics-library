# Formulas

This folder contains dedicated pages for individual formulas and named mathematical
functions.

Use the [Formula Registry](../formula-registry.md) as the main index. The registry
lists formulas without reproducing them; these pages hold the actual explanations.

## Pages

- [Cosine Function](cosine-function.md)
- [Harmonic And Prime Reciprocal Series](harmonic-and-prime-reciprocal-series.md)
- [L'Hopital's Rule](lhopitals-rule.md)
- [Lambert W Function](lambert-w-function.md)
- [Pythagorean Theorem](pythagorean-theorem.md)
- [Series That Sum To One](series-that-sum-to-one.md)
- [Square Root Of One Half](sqrt-one-half.md)
- [Taylor Series](taylor-series.md)
